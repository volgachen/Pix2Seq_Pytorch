...
def get_sha():
    ...
    try:
        ...
        status = "has uncommited changes" if diff else "clean"
        branch = _run(['git', 'rev-parse', '--abbrev-ref', 'HEAD'])
    except Exception:
        pass
    message = f"sha: {sha}, status: {status}, branch: {branch}" + f"\n{diff}" if diff else ""
    return message
...
def collate_fn(batch, fix_input=None):
    ...
    if "input_seq" in new_batch[1][0]:
        output_len = max([item['output_seq'].shape[0] for item in new_batch[1]])
        ...
        for idx, item in enumerate(new_batch[1]):
            i_seq, o_seq = item["input_seq"], item['output_seq']
